...
class DataIngestor:
    ...
    def states_mean(self, json_req):
        ...
        mean_vals = self.table[(self.table['Question'] == question) &
                               (self.table['YearStart'] >= 2011) &
                                (self.table['YearEnd'] <= 2022)]\
    ...
    def state_mean(self, json_req):
        ...
        mean_val = self.table[(self.table['Question'] == question) &
    ...
    def best5(self, json_req):
        ...
        return dict(sorted(mean_vals.to_dict().items(),
                            key = lambda pair: pair[1], reverse=True)[:5])
    ...
    def mean_by_category(self, json_req):
        ...
        grouped_vals = filtered_vals.groupby(['LocationDesc', 'StratificationCategory1',
    ...
    def state_mean_by_category(self, json_req):
        ...
        filtered_vals = self.table[(self.table['Question'] == question) &
                                    (self.table['LocationDesc'] == state)]
        grouped_vals = filtered_vals.groupby(['StratificationCategory1', 'Stratification1'])\
                                    ['Data_Value'].mean()
